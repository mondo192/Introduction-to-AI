% Programme: Farm Wolf Goat Cabbage Puzzle
...
solve(State, Goal, List) :-
	move(State, NextState), % move next state
	not(member(NextState, List)), % ensure state isnt on the List
	solve(NextState, Goal, [NextState | List]), % get the head of the list
